...
const CONTRACT_NAMES = ["FHECounter", "EncryptedSponsorDeal"];

// Point to this project's Hardhat root (one level up from frontend)
// Instead of the original monorepo path ../fhevm-hardhat-template
const rel = "..";
...
if (!fs.existsSync(dir)) {
  console.error(
    `${line}Unable to locate Hardhat project at '${rel}'. Expected to find crypto-sponsor-key root one level up from frontend.${line}`
  );
  process.exit(1);
}
...
function readDeployment(chainName, chainId, contractName, optional) {
  ...
  if (!fs.existsSync(contractPath)) {
    if (!optional) {
      console.error(
        `${line}Unable to locate '${contractPath}'.\n\n1. Goto '${dirname}' directory\n2. Run 'npx hardhat deploy --network ${chainName}'.${line}`
      );
      process.exit(1);
    }
    return undefined;
